...
namespace Voxis
{
	public static class GraphicsServer
	{
		...
		private static bool measurementRunning = false;
		private static uint queryTimeElapsedID = 0;
		private static uint queryPrimitivesGeneratedID = 1;
		...
		public static void StartPerformanceMeasurement()
		{
			...
			OpenGL.glCreateQueries(OpenGL.GL_TIME_ELAPSED, 1, &queryTimeElapsedID);
			BeginQuery(OpenGL.GL_TIME_ELAPSED, queryTimeElapsedID);

			OpenGL.glCreateQueries(OpenGL.GL_PRIMITIVES_GENERATED, 1, &queryPrimitivesGeneratedID);
			BeginQuery(OpenGL.GL_PRIMITIVES_GENERATED, queryPrimitivesGeneratedID);
		}

		public static void EndPerformanceMeasurement()
		{
			if(!measurementRunning)
			{
				measurementRunning = true;
				EndQuery(OpenGL.GL_TIME_ELAPSED, queryTimeElapsedID);
				EndQuery(OpenGL.GL_PRIMITIVES_GENERATED, queryPrimitivesGeneratedID);
			}
			UpdatePerformanceMeasurement();
		}

		public static void UpdatePerformanceMeasurement()
		{
			if (!measurementRunning) return;
			if (IsQueryResultAvailable(queryTimeElapsedID))
			{
				measurementRunning = false;
				Performance.CurrentFrame.GPUElapsedNanos = GetQueryResult(queryTimeElapsedID);
				OpenGL.glDeleteQueries(1, &queryTimeElapsedID);
			}
			if (IsQueryResultAvailable(queryPrimitivesGeneratedID))
			{
				measurementRunning = false;
				Performance.CurrentFrame.PrimitivesGenerated = GetQueryResult(queryPrimitivesGeneratedID);
				OpenGL.glDeleteQueries(1, &queryPrimitivesGeneratedID);
			}
		}

		public static void BeginQuery(uint targetQuery, uint id)
		{
			OpenGL.glBeginQuery(targetQuery, id);
		}
		public static void EndQuery(uint targetQuery, uint id)
		{
			OpenGL.glEndQuery(targetQuery);
		}
		public static bool IsQueryResultAvailable(uint id)
		{
			uint64 resultAvailable = OpenGL.GL_FALSE;
			OpenGL.glGetQueryObjectui64v(id, OpenGL.GL_QUERY_RESULT_AVAILABLE, &resultAvailable);
			return resultAvailable == OpenGL.GL_TRUE;
		}
		public static uint64 GetQueryResult(uint id)
		{
			uint64 result = 0;
			OpenGL.glGetQueryObjectui64v(id, OpenGL.GL_QUERY_RESULT, &result);
			return result;
		}
		...
		{
			if (enabled) OpenGL.glEnable(OpenGL.GL_SCISSOR_TEST);
			else OpenGL.glDisable(OpenGL.GL_SCISSOR_TEST);
		}
		...
		public static void DrawIndexedPrimitives(int indexCount, PrimitiveType primitiveType)
		{
			...
			{
			case .Lines:
				glDrawType = OpenGL.GL_LINES;
				break;
			case .Quads:
				glDrawType = OpenGL.GL_QUADS;
				break;
			case .Triangles:
				glDrawType = OpenGL.GL_TRIANGLES;
			default:
				Runtime.FatalError("Invalid draw type");
			}

			// Drawcall
			OpenGL.glDrawElements(glDrawType, indexCount, OpenGL.GL_UNSIGNED_SHORT, null);
		...
		public static void SetUniformBufferBinding(ShaderProgram program, StringView blockName, uint bindingPoint, UniformBuffer buffer)
		{
			uint bufferIndex = OpenGL.glGetUniformBlockIndex(program.ProgramID, blockName.ToScopeCStr!());

			OpenGL.glUniformBlockBinding(program.ProgramID, bufferIndex, bindingPoint);

			OpenGL.glBindBufferBase(OpenGL.GL_UNIFORM_BUFFER, bindingPoint, buffer.BufferID);
		}

		public static void UpdateUniformBuffer<T>(UniformBuffer buffer, T* data)
		{
			OpenGL.glBindBuffer(OpenGL.GL_UNIFORM_BUFFER, buffer.BufferID);
			OpenGL.glBufferSubData(OpenGL.GL_UNIFORM_BUFFER, 0, sizeof(T), data);
			OpenGL.glBindBuffer(OpenGL.GL_UNIFORM_BUFFER, 0);
		}

		public static UniformBuffer CreateUniformBuffer(int bufferSize)
		{
			uint buffer = 0;
			OpenGL.glGenBuffers(1, &buffer);

			OpenGL.glBindBuffer(OpenGL.GL_UNIFORM_BUFFER, buffer);
			OpenGL.glBufferData(OpenGL.GL_UNIFORM_BUFFER, bufferSize, null, OpenGL.GL_STATIC_DRAW);
			OpenGL.glBindBuffer(OpenGL.GL_UNIFORM_BUFFER, 0);

			return new [Friend]UniformBuffer(buffer);
		}
		...
		public static void DestroyResource(IGraphicsResource resource)
		{
			...
			else if (resource is Framebuffer)
			{
				DestroyFramebuffer((Framebuffer)resource);
			}
			else
			{
		...
		{
			uint bufferNum = 0;
			OpenGL.glGenFramebuffers(1, &bufferNum);

			Framebuffer buffer = new Framebuffer(bufferNum, uint(width), uint(height));

			BindFramebuffer(buffer);

			Texture2D colorAttachment = CreateTexture2D(width, height, .Linear, .RGB, .RGB, .UnsignedByte, null);
			Texture2D depthStencilAttachment = CreateTexture2D(width, height, .Linear, .Depth24Stencil8, .DepthStencil, .Uint24Uint8, null);

			buffer.ColorAttachment = colorAttachment;
			buffer.DepthStencilAttachment = depthStencilAttachment;

			OpenGL.glFramebufferTexture2D(OpenGL.GL_FRAMEBUFFER, OpenGL.GL_COLOR_ATTACHMENT0, OpenGL.GL_TEXTURE_2D, colorAttachment.TextureName, 0);
			OpenGL.glFramebufferTexture2D(OpenGL.GL_FRAMEBUFFER, OpenGL.GL_DEPTH_STENCIL_ATTACHMENT, OpenGL.GL_TEXTURE_2D, depthStencilAttachment.TextureName, 0);

			if (OpenGL.glCheckFramebufferStatus(OpenGL.GL_FRAMEBUFFER) != OpenGL.GL_FRAMEBUFFER_COMPLETE)
			{
				Runtime.FatalError("Framebuffer incomplete.");
			}
		...
		{
			uint bufferID = buffer.BufferName;
			OpenGL.glDeleteFramebuffers(1, &bufferID);
		}

		public static void BindFramebuffer(Framebuffer buffer)
		{
			if (buffer != null) OpenGL.glBindFramebuffer(OpenGL.GL_FRAMEBUFFER, buffer.BufferName);
			else OpenGL.glBindFramebuffer(OpenGL.GL_FRAMEBUFFER, 0);
		}
		...
		private static uint ToGLType(this BufferType type)
		{
			...
			case .Uint24Uint8:
				return OpenGL.GL_UNSIGNED_INT_24_8;
			}
		}
		...
		private static uint ToGlFormat(this PixelFormat format)
		{
			...
			case .Depth24Stencil8:
				return OpenGL.GL_DEPTH24_STENCIL8;
			case .DepthStencil:
				return OpenGL.GL_DEPTH_STENCIL;
			}
		}
