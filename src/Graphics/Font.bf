...
namespace Voxis
{
	public class Font
	{
		...
		public Color ShadowColor { get; set; } = .Black;
		public Vector2 ShadowOffset { get; set; } = Vector2(2.0f, 2.0f);
		...
		public void RenderTextEx(Vector2 bboxStart, StringView text, int carretLocation, int depth, float fontHeight = -1.0f)
		{
			...
			for(int i = 0; i < text.Length; i++)
			{
				...
				if (Shadow)
				{
					CanvasRenderPipeline.DrawTexturedRect(Rect(currentX + bear + ShadowOffset.X, y + ShadowOffset.Y, glyph.Texture.Width, glyph.Texture.Height), glyph.Texture, Rect(0, 0, glyph.Texture.Width, glyph.Texture.Height), depth, ShadowColor);
				}

				CanvasRenderPipeline.DrawTexturedRect(Rect(currentX + bear, y, glyph.Texture.Width, glyph.Texture.Height), glyph.Texture, Rect(0, 0, glyph.Texture.Width, glyph.Texture.Height), depth, .White);
		...
		public void DrawCharacter(Vector2 position, char32 codepoint, int depth, float fontHeight = -1.0f)
		{
			...
			if (Shadow)
			{
				CanvasRenderPipeline.DrawTexturedRect(Rect(position.X + bear + ShadowOffset.X, y + ShadowOffset.Y, glyph.Texture.Width, glyph.Texture.Height), glyph.Texture, Rect(0, 0, glyph.Texture.Width, glyph.Texture.Height), depth, ShadowColor);
			}

			CanvasRenderPipeline.DrawTexturedRect(Rect(position.X + bear, y, glyph.Texture.Width, glyph.Texture.Height), glyph.Texture, Rect(0, 0, glyph.Texture.Width, glyph.Texture.Height), depth, .White);
		}

		public void RenderText(Vector2 bboxStart, StringView text, int depth, float fontHeight = -1.0f)
		{
			float heightScaling = 1.0f;
			if (fontHeight > 0.0f)
			{
