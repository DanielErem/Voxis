...
namespace Voxis
{
	public static class CanvasRenderPipelineOld
	{
		...
		public static void End()
		{
			...
			for(BatchElement element in drawElements)
			{
				...
				if(element is BatchElementTexturedQuad)
				{
					...
					vertexArray.Add(Vertex2D(Vector2(quad.StartPosition.X, quad.StartPosition.Y), 0, textureIndex, quad.TexRegionStart, quad.Tint));
					vertexArray.Add(Vertex2D(Vector2(quad.EndPosition.X, quad.StartPosition.Y), 0, textureIndex, Vector2(quad.TexRegionEnd.X, quad.TexRegionStart.Y), quad.Tint));
					vertexArray.Add(Vertex2D(Vector2(quad.EndPosition.X, quad.EndPosition.Y), 0, textureIndex, quad.TexRegionEnd, quad.Tint));
					vertexArray.Add(Vertex2D(Vector2(quad.StartPosition.X, quad.EndPosition.Y), 0, textureIndex, Vector2(quad.TexRegionStart.X, quad.TexRegionEnd.Y), quad.Tint));
		...
		public static void DrawBlockModel(BlockModel model, Vector3 position, Quaternion rotation, Vector3 scale)
		{
			...
			{
				mesh.InsertQuad(quad, LightLevel(0,0,0,15), Vector3.Zero);
			}
			mesh.Upload(resMesh);

			RenderPipeline3D.DrawMesh(resMesh, projectionMatrix, view, transform, ChunckVertex.Layout, blockModelMaterial, null, environmentSettings);
