namespace Voxis
{
	public struct BlockPos
	{
		...
		{
			return ChunckIndex(int16(X >> Chunck.LOG_SIZE), int16(Z >> Chunck.LOG_SIZE));
		}

		public (int x, int y, int z) GetChunckLocalPosition()
		{
			return (X & Chunck.MASK, Y, Z & Chunck.MASK);
		}

		public BlockPos AddChunckIndexOffset(ChunckIndex index)
		{
			return BlockPos(X + index.X * Chunck.SIZE, Y, Z + index.Z * Chunck.SIZE);
		}
