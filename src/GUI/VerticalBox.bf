namespace Voxis
{
	public class VerticalBox : GUIElement
	{
		...
		public override Vector2 GetMinSize()
		{
			...
			// This is only the roundabout size with all of the children taken into account
			for (GUIElement element in childElements)
			{
				Vector2 childMinSize = element.GetMinSize();

				result.Y += childMinSize.Y;
				result.X = System.Math.Max(childMinSize.X, result.X);
			}

			// Add the paddings between the elements
			result.Y += Spacing * (childElements.Count - 1);
