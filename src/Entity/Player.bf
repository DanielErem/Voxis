...
namespace Voxis
{
	public class Player : Entity
	{
		...
		private Vector3 cameraOffset = Vector3(0, characterHeight / 2.0f - 0.1f, 0);
		...
		public ~this()
		{
			...
			delete creativeInventoryScreen;
		}
		...
		public override void OnInputEvent(InputEvent event)
		{
			...
			if (GUICanvas.IsBlockingInput()) return;
			...
			if (event is InputEventKeyboardKey)
			{
				...
				else if (keyEvent.Key == .KeyE && keyEvent.Action == .Press)
				{
					GUICanvas.ShowExclusive(creativeInventoryScreen);
				}
			}
			...
			if (event is InputEventMouseButton)
			{
				...
				if (mbEvent.Action != .Press) return;
		...
		public override void OnRender(Camera camera, CommandList commandList)
		{
			base.OnRender(camera, commandList);
			...
			if (Physics.TraceRay(World, LocalPosition + cameraOffset, Camera.Forward, 10.0f, ref hitPos, ref hitNormal))
			{
				...
				RenderPipeline3D.DrawRealDebugOutline(selectedPos.ToVector(), selectedPos.ToVector() + Vector3.One, Color.Black);
			}
		...
		public override void OnFixedUpdate()
		{
			...
			if (!GUICanvas.IsBlockingInput() && InputServer.IsKeyPressed(.Space))
			{
			...
			else if (!GUICanvas.IsBlockingInput() && InputServer.IsKeyPressed(.LeftShift))
			{
				if (IsFlying) CurrentVelocity = Vector3(CurrentVelocity.X, -flyingSpeed, CurrentVelocity.Z);
			}
			else
		...
		protected override void OnStart()
		{
			...
			creativeInventoryScreen.Player = this;
		}
